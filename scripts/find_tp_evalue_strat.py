...
#    return chunk.groupby(['query', 'evalue_bin'])[['pfam_label', 'clan_label']].sum()
...
#Please note that the chunks should not go above 250k which is the size of the target database. It is 10k by default
    chunk["evalue_bin"] = np.where(chunk['evalue'] == 0, -1000, np.round(np.log(chunk['evalue'])))
    ...
    head_chunk_result = find_tps_freq_in_batch(head_chunk)
