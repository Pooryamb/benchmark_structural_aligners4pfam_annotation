...
def run_scoring_function(inp_path):
    if "reseek" in inp_path:
        columns = rs_header
    elif "fs" in inp_path:
        columns = fs_header
#f"{base_dir}/tmp/alis/split_pf_seq/fs_pref_B1.tsv"
    ali_df = pd.read_csv(inp_path, sep="\t", header=None, names=columns)
    ali_df["t_family"] = ali_df["target"].str.split("-", expand=True)[3]
    ali_df["pssm_raw_score"] = ali_df.apply(get_ali_score, axis=1)
#.swifter.apply(get_ali_score, axis=1)
    ali_df = ali_df.drop(["qaln", "taln"], axis=1)
    ali_df.to_csv(inp_path.replace(".tsv", "_rescored.tsv"), sep="\t", index=None)
...
with ProcessPoolExecutor() as executor:
    executor.map(run_scoring_function, inp_paths)
